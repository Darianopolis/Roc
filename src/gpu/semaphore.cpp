...
core::Ref<gpu_semaphore> create_semaphore_base(gpu_context* gpu)
{
    ...
    gpu_check(gpu->vk.CreateSemaphore(gpu->device, core::ptr_to(VkSemaphoreCreateInfo {
        ...
        .pNext = core::ptr_to(VkSemaphoreTypeCreateInfo {
            .sType = VK_STRUCTURE_TYPE_SEMAPHORE_TYPE_CREATE_INFO,
            .pNext = core::ptr_to(VkExportSemaphoreCreateInfo {
...
core::Ref<gpu_semaphore> gpu_semaphore_create(gpu_context* gpu)
{
    ...
    int syncobj_fd = -1;
    gpu_check(gpu->vk.GetSemaphoreFdKHR(gpu->device, core::ptr_to(VkSemaphoreGetFdInfoKHR {
        .sType = VK_STRUCTURE_TYPE_SEMAPHORE_GET_FD_INFO_KHR,
        .semaphore = semaphore->semaphore,
        .handleType = VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_FD_BIT,
    }), &syncobj_fd));

    core::check<drmSyncobjFDToHandle>(gpu->drm.fd, syncobj_fd, &semaphore->syncobj);
...
core::Ref<gpu_semaphore> gpu_semaphore_import_syncobj(gpu_context* gpu, int syncobj_fd)
{
    ...
    if (gpu_check(gpu->vk.ImportSemaphoreFdKHR(gpu->device, core::ptr_to(VkImportSemaphoreFdInfoKHR {
...
void gpu_semaphore_import_syncfile(gpu_semaphore* semaphore, int sync_fd, u64 target_point)
{
    ...
    if (sync_fd == -1) {
        core::check<drmSyncobjTimelineSignal>(gpu->drm.fd, &semaphore->syncobj, &target_point, 1);
        return;
    }
    ...
    u32 syncobj = {};
    core::check<drmSyncobjCreate>(gpu->drm.fd, 0, &syncobj);
    defer { core::check<drmSyncobjDestroy>(gpu->drm.fd, syncobj); };
    core::check<drmSyncobjImportSyncFile>(gpu->drm.fd, syncobj, sync_fd);
...
int gpu_semaphore_export_syncfile(gpu_semaphore* semaphore, u64 source_point)
{
    ...
    core::check<drmSyncobjCreate>(gpu->drm.fd, 0, &syncobj);
    defer { core::check<drmSyncobjDestroy>(gpu->drm.fd, syncobj); };
    core::check<drmSyncobjTransfer>(gpu->drm.fd, syncobj, 0, semaphore->syncobj, source_point, 0);
    int sync_fd = -1;
    core::check<drmSyncobjExportSyncFile>(gpu->drm.fd, syncobj, &sync_fd);
...
void gpu_semaphore_wait(gpu_semaphore* semaphore, gpu_wait_fn* wait)
{
    ...
    if (!semaphore->wait.fd) {
        semaphore->wait.fd = core::fd::adopt(eventfd(0, EFD_CLOEXEC));

        core::fd::add_listener(semaphore->wait.fd.get(), gpu->event_loop.get(), core::FdEventBit::readable,
            [semaphore](int, core::Flags<core::FdEventBit>) {
    ...
    core::check<drmIoctl>(gpu->drm.fd, DRM_IOCTL_SYNCOBJ_EVENTFD, core::ptr_to(drm_syncobj_eventfd {
...
void gpu_wait(gpu_syncpoint syncpoint)
{
    ...
    gpu_check(gpu->vk.WaitSemaphores(gpu->device, core::ptr_to(VkSemaphoreWaitInfo {
...
void gpu_semaphore_signal_value(gpu_semaphore* semaphore, u64 value)
{
    ...
    gpu_check(gpu->vk.SignalSemaphore(gpu->device, core::ptr_to(VkSemaphoreSignalInfo {
